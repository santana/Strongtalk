...
klassOop createClass(symbolOop className, klassOop superclass) {
  ...
  return klassOop(systemPrimitives::createNamedInvocation(reinterpret_cast<oop>(format), superclassHandle.as_oop(),
                                                          trueObj, className, mixin));
}
...
TESTF(BehaviorPrimitivesSuperclassTests, setSuperclassShouldChangeSuperclassToNewClass) {
  ...
  ASSERT_TRUE_M(superclassOf(subclass()->klass()) == objectClass()->klass(), "Metasuperclass should be unchanged");
}
