...
public:
  ...
  int errorCount() { return errors->length(); }
  int warningCount() { return warnings->length(); }
  int compilerWarningCount() { return compilerWarnings->length(); }
  char* errorAt(int index) { return errors->at(index); }
  char* warningAt(int index) { return warnings->at(index); }
  char* compilerWarningAt(int index) { return compilerWarnings->at(index); }
